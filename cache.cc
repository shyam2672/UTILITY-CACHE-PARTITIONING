...
void atd_insert(PACKET *packet,int cpu,int set){
if(cpu==0){
int maxm=-1;
for(int i=0;i<16;i++){
maxm=max(int(maxm),int(atd1->arr[set][i].lru));
...

{
	...
	if(atd1->arr[set][i].tag==0)
	{

		atd1->arr[set][i].tag=packet->address;

	
    for(int j=0;j<i;j++)if(atd1->arr[set][j].tag>0)atd1->arr[set][i].lru++;
    	atd1->arr[set][i].lru=0;
		break;
...

{
  ...
  int maxm=-1;
for(int i=0;i<16;i++){
maxm=max(int(maxm),int(atd2->arr[set][i].lru));
...

{
	...
	if(atd2->arr[set][i].tag==0)
	{

		atd2->arr[set][i].tag=packet->address;

	
    for(int j=0;j<i;j++)if(atd2->arr[set][j].tag>0)atd2->arr[set][i].lru++;
    	atd2->arr[set][i].lru=0;
		break;
...
int atdupdate(PACKET *packet,int set,int cpu){
  if(cpu==0){
  int hitval ;
...

{
	...
	if(atd1->arr[set][i].tag==packet->address)

	{

hitval = atd1->arr[set][i].lru ;
		atd1->arr[set][i].lru = 0;
...

{
	...
	if(atd1->arr[set][i].lru < hitval and atd1->arr[set][i].tag!=0)
...

{
	...
	if(atd2->arr[set][i].tag==packet->address)

	{

hitval = atd2->arr[set][i].lru ;
		atd2->arr[set][i].lru = 0;
...

{
	...
	if(atd2->arr[set][i].lru < hitval and atd2->arr[set][i].tag!=0)
...
void atdfill(PACKET *packet,int cpu,uint32_t set){
if(cpu==0){
	if(set<64)
...
way=i;
	...
	if(flag)
		...
		atd1->umonglobalcounter[atdupdate(packet,set,cpu)]++;

	else

		atd_insert(packet,set,cpu);
...
way=i;
	...
	if(flag)
		...
		atd2->umonglobalcounter[atdupdate(packet,set,cpu)]++;
...
void partition(){
  int k=8;
  int maxm=-1;
  if(current_core_cycle[0]==0){
  ucp1=8;ucp2=8;return ;}
for(int i=1;i<=15;i++){
if(maxm<atd1->getumon(i)+atd2->getumon(16-i)){
maxm=atd1->getumon(i)+atd2->getumon(16-i);
k=i;
}
}
  
if(current_core_cycle[0]%2500000==0){
ucp1=k;ucp2=16-k;

for(int i=0;i<16;i++){
  atd1->umonglobalcounter[i]/=2;
  atd2->umonglobalcounter[i]/=2;
...

{
    ...
    if(set%64==0)
    atdfill(packet,packet->cpu,set/64);
    partition();
    int match_way = -1;
